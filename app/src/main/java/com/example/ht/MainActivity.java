                        ...
                        try {
                            ...
                            delayToshowProgess.postDelayed(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    tv_confirmed.setText(getInstance().format(parseInt(str_confirmed)));
                                    tv_confirmed_new.setText("+" + getInstance().format(parseInt(str_confirmed_new)));

                                    tv_active.setText(getInstance().format(parseInt(str_active)));

                                    int_active_new = parseInt(str_confirmed_new)
                                            - (parseInt(str_recovered_new) + parseInt(str_death_new));
                                    tv_active_new.setText("+"+ getInstance().format(int_active_new));

                                    tv_recovered.setText(getInstance().format(parseInt(str_recovered)));
                                    tv_recovered_new.setText("+"+ getInstance().format(parseInt(str_recovered_new)));

                                    tv_death.setText(getInstance().format(parseInt(str_death)));
                                    tv_death_new.setText("+"+ getInstance().format(parseInt(str_death_new)));

        //                            tv_tests.setText(getInstance().format(parseInt(str_tests)));
        //                            tv_tests_new.setText("+"+ getInstance().format(parseInt(str_tests_new)));

                                    tv_date.setText(FormatDate(str_last_update_time, 1));
                                    tv_time.setText(FormatDate(str_last_update_time, 2));

                                    pieChart.addPieSlice(new PieModel("Active", parseInt(str_active), Color.parseColor("#007afe")));
                                    pieChart.addPieSlice(new PieModel("Recovered", parseInt(str_recovered), Color.parseColor("#08a045")));
                                    pieChart.addPieSlice(new PieModel("Deceased", parseInt(str_death), Color.parseColor("#F6404F")));
